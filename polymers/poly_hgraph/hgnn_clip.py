...
class HierVAE(nn.Module):
    ...
    def forward(self, graphs, tensors, graph_blip, orders, text_input, beta=0.1, perturb_z=True):
        ...
        # # Handle cases where accuracy values might be scalars (when no predictions for that type)
        # wacc_val = wacc.item() if hasattr(wacc, 'item') else wacc
        # iacc_val = iacc.item() if hasattr(iacc, 'item') else iacc
        # tacc_val = tacc.item() if hasattr(tacc, 'item') else tacc
        # sacc_val = sacc.item() if hasattr(sacc, 'item') else sacc
        
        # return loss + beta * kl_div, kl_div.item(), wacc_val, iacc_val, tacc_val, sacc_val
