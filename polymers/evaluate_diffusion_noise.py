...
from diffusion.denoising_diffusion_noise import GaussianDiffusion, Trainer
...
# noise perturbation
parser.add_argument("--latent_noise_scale", type=float, default=0,
                   help="Scale of Gaussian noise to add to latent before decoder for defense")

# prune fragments
parser.add_argument("--prune_fragments", type=str, default=None,
                   help="Comma-separated fragment SMILES to prune from decoder, e.g., 'C1CS1,C1CO1'")
parser.add_argument("--backdoor_config", type=str, default=None,
                   help="Path to backdoor_candidates.json from detection phase")

args = parser.parse_args()

# Set up prune fragments list
prune_list = None
if args.prune_fragments:
    prune_list = [f.strip() for f in args.prune_fragments.split(',')]
elif args.backdoor_config:
    import json
    with open(args.backdoor_config) as f:
        config = json.load(f)
    prune_list = config.get('suspicious_fragments', [])
...
trainer = Trainer(
    ...
    prune_fragments = prune_list,
)
print("Finish Trainer")
print(f"Latent noise scale: {args.latent_noise_scale}")
print(f"Prune fragments: {prune_list}")
