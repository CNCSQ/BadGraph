...
# 如果指定了resume_dir且resume_training为True，加载已保存的diffusion模型进行微调
if args.resume_training and args.resume_dir is not None:
    print(f"Loading diffusion model from {args.resume_dir} for fine-tuning...")
    trainer.load(args.resume_dir)
    print(f"Loaded model at step {trainer.step}")
    # 可选：重置step从0开始，或继续从之前的step
    trainer.step = 0  # 取消注释此行以从step 0开始
