...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        mixed_precision = 'no',
        split_batches = True,
        latent_noise_scale = 0.0,  # latent噪声扰动比例（decoder阶段）
        prune_fragments = None,  # 要剪枝的fragment列表，如 ["C1CS1"]
    ):
        super().__init__()
        ...
        self.args = args
        self.latent_noise_scale = latent_noise_scale  # 保存latent噪声扰动比例
        self.prune_fragments = prune_fragments  # 保存要剪枝的fragment列表
    ...
    def apply_fragment_pruning(self):
        if self.prune_fragments is None or len(self.prune_fragments) == 0:
            return

        vocab = self.bart_model.decoder.vocab

        # 1. main
        cls_indices = []
        for target in self.prune_fragments:
            for i, cls in enumerate(vocab.hvocab):
                if cls == target or target in cls:
                    cls_indices.append(i)

        # 2. (main,sub) pair
        icls_indices = []
        for target in self.prune_fragments:
            for i, (cls, icls) in enumerate(vocab.vocab):
                if cls == target or target in cls:
                    icls_indices.append(i)

        cls_indices = list(set(cls_indices))
        icls_indices = list(set(icls_indices))

        with torch.no_grad():
            # clsNN
            if len(cls_indices) > 0:
                self.bart_model.decoder.clsNN[-1].bias[cls_indices] = -1e9
                print(f"[Defense] Pruned clsNN: {len(cls_indices)} class indices: {cls_indices}")

            # iclsNN
            if len(icls_indices) > 0:
                self.bart_model.decoder.iclsNN[-1].bias[icls_indices] = -1e9
                print(f"[Defense] Pruned iclsNN: {len(icls_indices)} subclass indices: {icls_indices}")

        if len(cls_indices) > 0 or len(icls_indices) > 0:
            print(f"[Defense] Pruned fragments: {self.prune_fragments}")
        else:
            print(f"[Defense] Warning: No matching indices found for fragments: {self.prune_fragments}")
    ...
    def evaluate_test_multi(self, loader, part=True, n_samples=5, w = 0.9):
        ...
        with torch.no_grad():
            with open(f'./results_{current_time}.txt', 'w') as fin:
                fin.write("description\tground truth\toutput\n")
                print(len(loader), "------------")
                for i,batch in enumerate(loader):
                    if count ==3 and part:
                    ...
                    for t in range(0, n_samples):
                        ...
                        # 在decoder之前对latent添加噪声扰动用于防御
                        if self.latent_noise_scale > 0:
                            latent_noise = torch.randn_like(latents) * self.latent_noise_scale
